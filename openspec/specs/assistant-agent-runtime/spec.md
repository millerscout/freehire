# assistant-agent-runtime Specification

## Purpose

The in-app AI agent: how a turn runs, what it may call, and what it may never do.
The agent executes inside the freehire backend as the authenticated caller, so its
